...
static med_entry_t *medredis_reply_to_entry(redisReply *reply, const char* cid, const char* key) {
    ...
    if (reply->element[3]->type != REDIS_REPLY_STRING) {
        L_WARNING("Received Redis reply type %i instead of %i (string) for call-id field of cid '%s' using key '%s', trashing record\n",
                reply->element[i]->type, REDIS_REPLY_STRING, cid, key);
        medredis_remove_mappings(cid, key);
        medredis_remove_entry(key);
        return NULL;
    }
    ...
    if (reply->element[0]->type != REDIS_REPLY_STRING) {
        L_WARNING("Received Redis reply type %i instead of %i (string) for sip_code field of cid '%s' using key '%s'\n",
                reply->element[0]->type, REDIS_REPLY_STRING, cid, key);
        entry->valid = 0;
    } else {
    ...
    if (reply->element[1]->type != REDIS_REPLY_STRING) {
        L_WARNING("Received Redis reply type %i instead of %i (string) for sip_reason field of cid '%s' using key '%s'\n",
                reply->element[1]->type, REDIS_REPLY_STRING, cid, key);
        entry->valid = 0;
    } else {
    ...
    if (reply->element[2]->type != REDIS_REPLY_STRING) {
        L_WARNING("Received Redis reply type %i instead of %i (string) for sip_method field of cid '%s' using key '%s'\n",
                reply->element[2]->type, REDIS_REPLY_STRING, cid, key);
        entry->valid = 0;
    } else {
    ...
    if (reply->element[4]->type != REDIS_REPLY_STRING) {
        L_WARNING("Received Redis reply type %i instead of %i (string) for timestamp field of cid '%s' using key '%s'\n",
                reply->element[4]->type, REDIS_REPLY_STRING, cid, key);
        entry->valid = 0;
    } else {
    ...
    if (reply->element[6]->type != REDIS_REPLY_STRING) {
        L_WARNING("Received Redis reply type %i instead of %i (string) for src_leg field of cid '%s' using key '%s'\n",
                reply->element[6]->type, REDIS_REPLY_STRING, cid, key);
        entry->valid = 0;
    } else {
    ...
    if (reply->element[7]->type != REDIS_REPLY_STRING) {
        L_WARNING("Received Redis reply type %i instead of %i (string) for dst_leg field of cid '%s' using key '%s'\n",
                reply->element[7]->type, REDIS_REPLY_STRING, cid, key);
        entry->valid = 0;
    } else {
...
/**********************************************************************/
static int medredis_remove_entry(const char* key) {
    char *argv[2];
