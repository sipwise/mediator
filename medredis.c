...
static int medredis_cleanup_entries(med_entry_t *records, uint64_t count, const char *table) {
    ...
    for (uint64_t i = 0; i < count; ++i) {
        ...
        if (e->branch_id.size() == 0) {
            L_CRITICAL("branch_id size is 0\n");
        }
        if (e->branch_id.empty() {
            L_CRITICAL("branch_id is empty\n");
        }
        if (e->branch_id.compare("") == 0) {
            L_CRITICAL("branch_id compare to null string is ok\n");
