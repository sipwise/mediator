...
#define medredis_check_reply(con, cmd, reply, err) do { \
    if (!(reply) && !(con)->ctx) { \
        syslog(LOG_ERR, "Failed to perform redis query '%s': no connection to server\n", \
        	(cmd)); \
        goto err; \
    } \
    if (!(reply)) { \
        syslog(LOG_ERR, "Failed to perform redis query '%s': %s\n", \
                (cmd), (con)->ctx->errstr); \
        goto err; \
    } \
...
static med_entry_t *medredis_reply_to_entry(redisReply *reply) {
	...
	if (reply->elements != 8) {
		syslog(LOG_ERR, "Invalid number of redis reply elements for acc record, expected 8, got %lu\n",
			reply->elements);
		return NULL;
	}
	entry = (med_entry_t*)malloc(sizeof(*entry));
	...
	g_strlcpy(entry->timestamp,  reply->element[4]->str, sizeof(entry->method));
...
static void medredis_dump_reply(redisReply *reply) {
    ...
    if (reply->type == REDIS_REPLY_STRING) {
        ...
        syslog(LOG_DEBUG, "printing %lu elements in array reply\n", reply->elements);
        for(i = 0; i < reply->elements; ++i) {
            medredis_dump_reply(reply->element[i]);
        }
    } else {
        syslog(LOG_DEBUG, "  not printing invalid reply type\n");
    }
}

/**********************************************************************/
static void medredis_dump_command(size_t argc, char **argv) {
	syslog(LOG_DEBUG, "Dumping command:\n");
	for (size_t i = 0; i < argc; ++i) {
		syslog(LOG_DEBUG, "  %lu: %s", i, argv[i]);
	}
}
...
static int medredis_push_command(medredis_con_t *con, size_t argc, char **argv) {
    ...
    if (!cmd) {
        syslog(LOG_ERR, "Failed to allocate memory for redis command\n");
        goto err;
    }
    cmd->argc = 0;
    cmd->argv = (char**)malloc(argc * sizeof(char*));
    if (!cmd->argv) {
    	syslog(LOG_ERR, "Failed to allocate memory for redis query\n");
        goto err;	
    }
    for (size_t i = 0; i < argc; ++i) {
...
static redisReply *medredis_command(medredis_con_t *con, const char* cmd) {
	syslog(LOG_DEBUG, "Performing redis query '%s'\n", cmd);
    redisReply *reply = redisCommand(con->ctx, cmd);
    if (con->ctx->err == REDIS_ERR_EOF) {
        if (medredis_init() != 0) {
            syslog(LOG_ERR, "Failed to reconnect to redis db\n");
            medredis_cleanup();
            return NULL;
        }
...
static int medredis_append_command_argv(medredis_con_t *con, size_t argc, char **argv, int queue) {
    ...
    if (queue > 0 && medredis_push_command(con, argc, argv) != 0) {
        syslog(LOG_ERR, "Failed to queue redis command\n");
        return -1;
    }
...
static int medredis_get_reply(medredis_con_t *con, redisReply **reply) {
    ...
    *reply = NULL;
    ...
    if (con->ctx->err == REDIS_ERR_EOF) {
        syslog(LOG_DEBUG, "Redis connection is gone, try reconnect\n");
        ...
        while ((cmd = medredis_shift_command(con))) {
            syslog(LOG_DEBUG, "re-queueing appended command\n");
            if (medredis_append_command_argv(con, cmd->argc, cmd->argv, 0) != 0) {
                syslog(LOG_ERR, "Failed to re-queue redis command");
                return -1;
            }
            medredis_free_command(&cmd);
        }
        ...
        syslog(LOG_DEBUG, "Redis get_reply successful, popping command\n");
...
int medredis_init() {
    ...
    syslog(LOG_DEBUG, "Connecting to redis %s:%d/%d\n",
    	config_redis_host, config_redis_port, config_redis_db);

    con = (medredis_con_t*)malloc(sizeof(medredis_con_t));
    if (!con) {
    	syslog(LOG_ERR, "Failed to allocate memory for redis connection\n");
    	goto err;
    }
    ...
    if (!con->ctx) {
        syslog(LOG_ERR, "Cannot open redis connection to %s:%d\n",
        	config_redis_host, config_redis_port);
        goto err;
    }
    if (con->ctx->err) {
        syslog(LOG_ERR, "Cannot open redis connection to %s:%d: %s\n",
        	config_redis_host, config_redis_port,
            con->ctx->errstr);
        goto err;
    }
    ...
    syslog(LOG_DEBUG, "Redis connection opened to %s:%d/%d\n",
    	config_redis_host, config_redis_port, config_redis_db);
...
static void medredis_add_cid(void *key, void *val, void *data) {
	...
	syslog(LOG_DEBUG, "Adding cid %s [%s] at index %"PRIu64"\n", (char*)key, (char*)val, list->index);
...
med_callid_t *medredis_fetch_callids(uint64_t *count) {
	...
	*count = 0;
	...
	do {
		...
		if (reply->type != REDIS_REPLY_ARRAY) {
            syslog(LOG_ERR, "Invalid reply type for scan, expected array\n");
            goto err;
        }
        if (reply->elements != 2) {
            syslog(LOG_ERR, "Invalid number of reply elements for scan, expected 2, got %lu\n",
                    reply->elements);
            goto err;
        }
        ...
        if (reply->element[0]->type == REDIS_REPLY_STRING) {
            ...
            syslog(LOG_ERR, "Invalid cursor type for scan, expected string or integer\n");
            goto err;
        }

        if (reply->element[1]->type != REDIS_REPLY_ARRAY) {
            syslog(LOG_ERR, "Invalid content type for scan, expected array\n");
            goto err;
        }
        if (reply->element[1]->elements == 0 && cursor == 0) {
            syslog(LOG_DEBUG, "No matching entries found for scan\n");
            medredis_free_reply(&reply);
            break;
        }
        ...
        for (i= 0; i < reply->element[1]->elements; ++i) {
            ...
            if (!entry) {
                syslog(LOG_ERR, "Invalid null entry at cursor result index %lu while scanning table\n", i);
                goto err;
            }
            if (entry->type != REDIS_REPLY_STRING) {
                syslog(LOG_ERR, "Invalid entry type at cursor result index %lu while scanning table, expected string\n", i);
                goto err;
            }

            syslog(LOG_DEBUG, "Got entry '%s'\n", entry->str);
	...
	*count = g_hash_table_size(cid_table);
	...
	if (!entries) {
		syslog(LOG_ERR, "Failed to allocate memory for callid list\n");
		goto err;
	}
...
static void medredis_append_key(gpointer data, gpointer user_data) {
	...
	syslog(LOG_DEBUG, "Appending key '%s' to keys list\n", key);
    ...
    if (medredis_append_command_argv(con, entry_argc, entry_argv, 1) != 0) {
    	syslog(LOG_ERR, "Failed to append command to fetch key\n");
    }
    free(key);
}
...
int medredis_fetch_records(med_callid_t *callid,
	...
	*/
    ...
    if (!cids[0]) {
    	syslog(LOG_ERR, "Failed to allocate memory for callid\n");
    	goto err;
    }
    snprintf(buffer, sizeof(buffer), "%s%s", callid->value, PBXSUFFIX);
    cids[1] = strdup(buffer);
    if (!cids[1]) {
    	syslog(LOG_ERR, "Failed to allocate memory for callid with pbxsuffix\n");
    	goto err;
    }
    snprintf(buffer, sizeof(buffer), "%s%s", callid->value, XFERSUFFIX);
    cids[2] = strdup(buffer);
    if (!cids[2]) {
    	syslog(LOG_ERR, "Failed to allocate memory for callid with xfersuffix\n");
    	goto err;
    }
	...
	*entries = NULL;
	...
	syslog(LOG_DEBUG, "Fetching records from redis\n");
	...
	for (i = 0; i < 3; ++i) {
		...
		if (medredis_append_command_argv(con, cid_set_argc, cid_set_argv, 1) != 0) {
			syslog(LOG_ERR, "Failed to append redis command to fetch entries for cid '%s'\n", callid->value);
			goto err;
		}
		free(cid);
	}
	...
	for (i = 0; i < 3; ++i) {
		if (medredis_get_reply(con, &reply) != 0) {
			syslog(LOG_ERR, "Failed to get redis reply for command to fetch entries for cid '%s'\n", callid->value);
			goto err;	
		}
        ...
        if (!reply->elements) {
        	syslog(LOG_DEBUG, "No matching entries for cid '%s' at suffix idx %lu\n", callid->value, i);
        	continue;
        }
        ...
        for (size_t j = 0; j < reply->elements; ++j) {
        	...
        	if (!key) {
        		syslog(LOG_ERR, "Failed to allocate memory for redis key\n");
        		goto err;
        	}
        	syslog(LOG_DEBUG, "Putting key '%s' to keys list\n", key);
    ...
    syslog(LOG_DEBUG, "Appending all keys to redis command\n");
    g_list_foreach(keys, medredis_append_key, NULL);
    do {
    	med_entry_t *e;
    	syslog(LOG_DEBUG, "Fetching next reply record\n");
    	if (medredis_get_reply(con, &reply) != 0) {
    		syslog(LOG_ERR, "Failed to get reply from redis\n");
    		goto err;
    	}
    	if (!reply)
    		break;
    	...
    	if (!e) {
    		syslog(LOG_ERR, "Failed to convert redis reply to entry\n");
    		goto err;
    	}
    ...
    *entries = (med_entry_t*)malloc(*count * sizeof(med_entry_t*));
    if (!*entries) {
    	syslog(LOG_ERR, "Failed to allocate memory for entries\n");
    	goto err;
    }
    ...
    for (GList *l = records; l; l = l->next) {
    	...
    	if (s->sip_method[0] == 'I' && s->sip_method[1] == 'N' && s->sip_method[2] == 'V' &&
    			s->sip_code[0] == '2') {
    		has_inv_200 |= 1;
    	} else if (s->sip_method[0] == 'B') {
    		has_bye |= 1;
    	}
    	entries[i++] = s;
    }
    if (has_inv_200 && !has_bye) {
    	syslog(LOG_DEBUG, "Found incomplete call with cid '%s', skipping...\n", callid->value);
    	free(*entries);
    	*entries = NULL;
    	*count = 0;
    }
...
static int medredis_rename_entries(const char *callid, const char *type) {
    ...
    for (i = 0; i < 3; ++i) {
		...
		for (j = 0; j < reply->elements; ++j) {
			...
			if (medredis_append_command_argv(con, 3, argv, 1) != 0) {
				syslog(LOG_ERR, "Failed to append redis command to rename entries for cid '%s'\n", callid);
				goto err;
			}
			...
			argv[2] = key;
			if (medredis_append_command_argv(con, 3, argv, 1) != 0) {
				syslog(LOG_ERR, "Failed to append redis command to remove key '%s' from acc:meth::INVITE\n", key);
				goto err;
			}
			argv[1] = "acc:meth::BYE";
			if (medredis_append_command_argv(con, 3, argv, 1) != 0) {
				syslog(LOG_ERR, "Failed to append redis command to remove key '%s' from acc:meth::BYE\n", key);
				goto err;
			}
		}
		medredis_free_reply(&reply);
	...
	syslog(LOG_DEBUG, "Waiting for final replies for cid '%s' after cleanup\n", callid);
	medredis_consume_replies(con);
	...
	syslog(LOG_DEBUG, "Done renaming '%s' for cleanup\n", callid);
	return 0;
