...
static int cdr_parse_srcleg_json(json_object *json, cdr_entry_t *cdr)
{
    ...
    if (!cdr_parse_json_get_strbuf(json, "cc_quota", cdr->source_concurrent_calls_quota)) {
        L_ERROR("Call-Id '%s' does not contain 'cc_quota' key (source concurrent calls quota), '%s'", cdr->call_id, json_object_get_string(json));
        goto err;
    }

    // source_concurrent_calls_count
    if (!cdr_parse_json_get_strbuf(json, "cc_sub", cdr->source_concurrent_calls_count)) {
        L_ERROR("Call-Id '%s' does not contain 'cc_sub' key (source concurrent calls count), '%s'", cdr->call_id, json_object_get_string(json));
        goto err;
    }
    ...
    if (!cdr_parse_json_get_strbuf(json, "cc_cust", cdr->source_concurrent_calls_count_customer)) {
        L_ERROR("Call-Id '%s' does not contain 'cc_cust' key (source concurrent calls count customer), '%s'", cdr->call_id, json_object_get_string(json));
        goto err;
    }
...
static int cdr_parse_dstleg_json(json_object *json, cdr_entry_t *cdr)
{
    ...
    if (!cdr_parse_json_get_strbuf(json, "cc_quota", cdr->destination_concurrent_calls_quota)) {
        L_ERROR("Call-Id '%s' does not contain 'cc_quota' key (destination concurrent calls quota), '%s'", cdr->call_id, json_object_get_string(json));
        goto err;
    }

    // destination_concurrent_calls_count
    if (!cdr_parse_json_get_strbuf(json, "cc_sub", cdr->destination_concurrent_calls_count)) {
        L_ERROR("Call-Id '%s' does not contain 'cc_sub' key (destination concurrent calls count), '%s'", cdr->call_id, json_object_get_string(json));
        goto err;
    }
    ...
    if (!cdr_parse_json_get_strbuf(json, "cc_cust", cdr->destination_concurrent_calls_count_customer)) {
        L_ERROR("Call-Id '%s' does not contain 'cc_cust' key (destination concurrent calls count customer), '%s'", cdr->call_id, json_object_get_string(json));
        goto err;
    }
