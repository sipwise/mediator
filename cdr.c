...
static int cdr_parse_json_get_uint8t(json_object *obj, const char *key, uint8_t *outp) {
    json_object *int_obj;
...
static int cdr_parse_json_get_uint8t_limit(json_object *obj, const char *key, uint8_t *outp, uint8_t min, uint8_t max) {
    if(!cdr_parse_json_get_uint8t(obj, key, outp))
        return 0;
    if (*outp > max)
        *outp = max;
    else if (*outp < min)
...
static int cdr_parse_json_get_uint64t(json_object *obj, const char *key, uint64_t *outp) {
    json_object *int_obj;
    if (!json_object_object_get_ex(obj, key, &int_obj))
        return 0;
    if (!json_object_is_type(int_obj, json_type_int))
...
static int cdr_parse_json_get_uint64t_limit(json_object *obj, const char *key, uint64_t *outp, uint64_t min, uint64_t max) {
    if(!cdr_parse_json_get_uint64t(obj, key, outp))
        return 0;
    if (*outp > max)
        *outp = max;
    else if (*outp < min)
...
static int cdr_parse_json_get_int_limit(json_object *obj, const char *key, int *outp, int min, int max) {
...
static int cdr_parse_json_get_double_limit(json_object *obj, const char *key, double *outp, double min, double max) {
...
static int cdr_parse_srcleg(char *srcleg, cdr_entry_t *cdr)
{
    ...
    // source_account_id
    if (!cdr_parse_json_get_uint64t(json, "a_id", &cdr->source_account_id)) {
    ...
    // source_clir
    if (!cdr_parse_json_get_uint8t_limit(json, "clir", &cdr->source_clir, 0, 1)) {
    ...
    if (!cdr_parse_json_get_double(json, "t", &cdr->init_time)) {
        L_ERROR("Call-Id '%s' does not contain 't' key (source init time), '%s'", cdr->call_id, srcleg);
        goto err;
    }
    ...
    // source_lcr_id
    if (!cdr_parse_json_get_uint64t(json, "lcr", &cdr->source_lcr_id)) {
...
static int cdr_parse_dstleg(char *dstleg, cdr_entry_t *cdr)
{
    ...
    // split
    if (!cdr_parse_json_get_uint8t(json, "plu", &cdr->split)) {
    ...
    if (!cdr_parse_json_get_uint64t(json, "a_id", &cdr->destination_account_id)) {
    ...
    // destination_lcr_id
    if (!cdr_parse_json_get_uint64t(json, "lcr", &cdr->destination_lcr_id)) {
...
static int cdr_parse_bye_dstleg(char *dstleg, mos_data_t *mos_data) {
    ...
    if (!cdr_parse_json_get_double_limit(mos, "avg_score", &mos_data->avg_score, 0, 99)) {
        L_ERROR("JSON object does not contain 'mos.avg_score' key: '%s'", dstleg);
        goto err;
    }
    if (!cdr_parse_json_get_int_limit(mos, "avg_packetloss", &mos_data->avg_packetloss, 0, 100)) {
        L_ERROR("JSON object does not contain 'mos.avg_packetloss' key: '%s'", dstleg);
        goto err;
    }
    if (!cdr_parse_json_get_int_limit(mos, "avg_jitter", &mos_data->avg_jitter, 0, 9999)) {
        L_ERROR("JSON object does not contain 'mos.avg_jitter' key: '%s'", dstleg);
        goto err;
    }
    if (!cdr_parse_json_get_int_limit(mos, "avg_rtt", &mos_data->avg_rtt, 0, 9999)) {
