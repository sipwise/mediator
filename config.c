...
static const char options[] = "?D:i:dlh:u:p:b:o:H:U:P:B:O:S:T:R:A:N:Z:z:W:w:X:x:ms";
...
static int config_parse_line(char *line)
{
	...
	syslog(LOG_DEBUG, "config file line '%s'", line);
	...
	if (option->name == NULL) {
		syslog(LOG_ERR, "config file line '%s', no option found", line);
		return -1;
	}

	if (option->has_arg == no_argument && *value != '\0') {
		syslog(LOG_ERR, "config file option '%s' excess argument",
		       option->name);
		return -1;
	} else if (option->has_arg == required_argument && *value == '\0') {
		syslog(LOG_ERR, "config file option '%s' missing argument",
		       option->name);
		return -1;
	}
	...
	syslog(LOG_DEBUG, "config file option %s (%d) = '%s'",
	       option->name, option->val, value);
...
static int config_parse_file(const char *filename)
{
	...
	syslog(LOG_DEBUG, "config file loading '%s", filename);
	...
	if (conffile == NULL) {
		...
		syslog(LOG_ERR, "config file, load error: %s", strerror(errno));
		return -1;
	}
	...
	while ((nread = getline(&line, &len, conffile)) >= 0) {
		...
		if (config_parse_line(line) < 0) {
			syslog(LOG_ERR, "config file, parse error");
			return -1;
		}
	}
