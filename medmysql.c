...
static int medmysql_mos_record(GQueue *q, unsigned long cdr_id, double mos_min, double mos_max,
		double mos_average)
{
	cdr_tag_record *record = malloc(sizeof(*record));
	record->cdr_id = cdr_id;
	if (asprintf(&record->sql_record, "%f, %f, %f",
				mos_min, mos_max, mos_average) <= 0)
		return -1;
	g_queue_push_tail(q, record);
	return 0;
}
...
//			return -1;
//		if (medmysql_mos_record(&batches->cdr_mos, batches->num_cdrs, mos_min, mos_max, mos_average))
//			return -1;
