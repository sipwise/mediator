...
int medmysql_trash_entries(const char *callid)
{
	...
	if (medmysql_query_wrapper_tx(med_handler, statement, stm_len))
		return -1;
...
int medmysql_insert_cdrs(cdr_entry_t *entries, u_int64_t count)
{
	...
	for(i = 0; i < count; ++i)
	{
		...
		if(medmysql_query_wrapper_tx(cdr_handler, statement, stm_len) != 0)
		{
...
static int medmysql_flush_call_stat_info() {
	...
	for (iter = keys; iter != NULL; iter = iter->next) {
		...
		if(medmysql_query_wrapper(stats_handler, statement, stm_len) != 0)
		{
