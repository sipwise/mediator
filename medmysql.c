...
static unsigned int mysql_real_query_errno(MYSQL *m, const char *s, unsigned long len) {
...
static int mysql_query_wrapper(mysql_handler *mysql, const char *stmt_str, unsigned long length) {
	...
	for (i = 0; i < 10; i++) {
		err = mysql_real_query_errno(mysql->m, stmt_str, length);
		if (!err)
			break;
		...
		syslog(LOG_WARNING, "Got fatal error from SQL (statement '%.*s'): %s",
				(int) length, stmt_str, mysql_error(mysql->m));
		break;
	}
	return !!err;
}
	...
	for (i = 0; i < 10; i++) {
				...
				|| err == ER_LOCK_DEADLOCK)
		{
			...
			syslog(LOG_WARNING, "Got error %u from SQL server during transaction, retrying...",
					err);
			err = mysql_real_query_errno(mysql->m, "rollback", 8);
			if (err) {
...
static int mysql_handler_transaction(mysql_handler *h) {
	if (h->is_transaction)
		return 0;
	if (mysql_query_wrapper(h, "start transaction", 17))
		return -1;
	h->is_transaction = 1;
	return 0;
}

static int mysql_handler_commit(mysql_handler *h) {
	if (!h->is_transaction)
		return 0;
...
int medmysql_batch_end() {
	...
	if (mysql_handler_commit(cdr_handler))
		return -1;
	if (mysql_handler_commit(med_handler))
		return -1;
