...
static int medmysql_flush_med_str(struct medmysql_str *str, medmysql_handler *handler) {
	if (str->len == 0)
		return 0;
	...
	if (medmysql_query_wrapper_tx(cdr_handler, str->str, str->len) != 0) {
		str->len = 0;
		syslog(LOG_CRIT, "Error executing query: %s",
				mysql_error(handler->m));
		return -1;
	}
...
static int medmysql_flush_cdr(struct medmysql_batches *batches) {
	...
	if (medmysql_flush_med_str(&batches->cdrs, cdr_handler)) {
...
static int medmysql_flush_medlist(struct medmysql_str *str) {
	if (medmysql_flush_med_str(str, med_handler)) {
